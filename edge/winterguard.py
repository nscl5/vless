...
def main():
    try:
        ...
        for i in range(NUM_PROXY_PAIRS):
            ...
            proxies_list.append(yaml.Node.merge(dialer_proxy, base_dialer_config) if False else {**dialer_proxy})
            # Python standard way to force Anchor reference using internal mapping trick
            proxies_list[-1] = dialer_proxy
            dialer_proxy.update({k: v for k, v in base_dialer_config.items() if k not in dialer_proxy})
            ...
            entry_proxy.update({k: v for k, v in base_entry_config.items() if k not in entry_proxy})
        ...
        masque_01.update({k: v for k, v in base_masque_config.items() if k not in masque_01})
        ...
        masque_02.update({k: v for k, v in base_masque_config.items() if k not in masque_02})
        ...
        class ClashMetaDumper(yaml.SafeDumper):
            def ignore_aliases(self, data):
                ...
                if isinstance(data, dict) and data.get("name") in ["TEMPLATE-IR-DIALER", "TEMPLATE-EU-ENTRY", "TEMPLATE-MASQUE"]:
                    return False
